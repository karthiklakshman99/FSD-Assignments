...
namespace Test
{
    public static class SeedData
    {
        ...
        {
            context.Categories.Add(new Category { CategoryName = "Testing", CategoryDescription = "Unit Testing", CategoryCreatedBy = 8105845 });
            context.SaveChanges();
        }
        private static void AddReminders(KeepDbContext context)
        {
            context.Reminders.Add(new Reminder { ReminderName = "Email", ReminderDescription = "Email reminder", ReminderType = "notification", ReminderCreatedBy = 8105845 });
            context.SaveChanges();
        }
        private static void AddUsers(KeepDbContext context)
        {
            context.Users.Add(new User { UserId = 8105845, UserName = "John Simon", Password = "test123", Contact = "9812345670" });
            context.SaveChanges();
        }
